    ...
    <div class="ratings-content">
      ...
      <div class="ratingselect" style="pointer-events: auto;">
        <div class="rating-type border-1px">
          <span class="block positive " :class="{active:selectType===2}" @click="selectTyped(2)">
            全部
            <span class="count">{{ratings.length}}</span>
          </span>
          <span class="block positive" @click="selectTyped(0)" :class="{active:selectType===0}">
            满意
            <span class="count">{{positiveSize}}</span>
          </span>
          <span class="block negative"  @click="selectTyped(1)" :class="{active:selectType===1}">
            不满意
          ...
          <li class="rating-item" v-for="(item,index) in  filterRatings" :key="index">
            ...
            <div class="content">
              ...
              <div class="recommend">
                ...
                <span class="item" v-for="(result,index) in item.recommend" :key="index">{{result}}</span>
              </div>
              <!-- 使用自定义过滤器 |方法名 -->
              <div class="time">{{item.rateTime | dataDate}}</div>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  data () {
    return {
      onlyText: true, // 是否有内容显示
      selectType: 2 // 选择的评价类：0满意 1不满意 2全部
    }
  },
  mounted () {
    this.$store.dispatch('bizRatings', () => {
      this.$nextTick(() => {
        // eslint-disable-next-line no-new
        new BCscroll(this.$refs.ratings, {
          click: true
        })
      })
    })
